# Barry faces Cutthroat Dave - Book 1 Moment Illustration

Public manual prompt for ChatGPT Images. Attach the listed portraits before generating the moment illustration.

## Target File

`illustration.webp`

## Moment

- Chapter: `ch4`
- Trigger scene: `Ch4-Barry-vs-Dave`
- Moment id: `ch4-barry-vs-dave`

## Character References To Attach

- Barry: `public/visuals/book1/characters/barry/portrait.webp` as `barry.webp`
- Kate: `public/visuals/book1/characters/kate/portrait.webp` as `kate.webp`
- Cutthroat Dave: `public/visuals/book1/characters/cutthroat-dave/portrait.webp` as `cutthroat-dave.webp`

## Scene Facts

- Setting: Forest fight space with trees close enough for cover, dirty ground and the aftermath of earlier knife violence.
- Action: Barry confronts Dave while Kate's dangerous presence or ice magic shifts the balance.
- Materials and architecture: Barry has ordinary travel gear only; Dave's knives and rough clothes remain central.
- Unnamed figures: none
- Composition: 16:9 confrontation, Barry visibly scared but determined, Dave threatening, Kate as cold controlled pressure.
- Timeline overrides: Barry still has no crossbow; keep him under-equipped and fallible.
- Path compatibility: Barry and Kate's exact injury state depends on previous choices, so keep wounds subtle and non-specific.
- Barry equipment continuity: Barry's stat device must stay in his pocket or hidden for this scene; do not put the device in his hands and do not make it a focal prop.
- Barry equipment continuity: Barry's backpack is not enchanted yet for this visual moment; show a plain weathered travel backpack with no magical glow, no magical seams and no inventory screen.
- Avoid: No heroic armor for Barry, no later crossbow, no excessive gore.

## Prompt ChatGPT

```text
Create one Book 1 moment illustration for Magium: Barry faces Cutthroat Dave.

Attached references:
- `barry.webp` = Barry
- `kate.webp` = Kate
- `cutthroat-dave.webp` = Cutthroat Dave

Use the attached references for stable identity, species, body shape, clothing and magical aura, but obey the scene-specific overrides below.

Scene-specific overrides:
- Barry still has no crossbow; keep him under-equipped and fallible.

Path compatibility:
- Barry and Kate's exact injury state depends on previous choices, so keep wounds subtle and non-specific.

Barry equipment continuity:
- Barry's stat device must stay in his pocket or hidden for this scene; do not put the device in his hands and do not make it a focal prop.
- Barry's backpack is not enchanted yet for this visual moment; show a plain weathered travel backpack with no magical glow, no magical seams and no inventory screen.

Scene design:
- Setting: Forest fight space with trees close enough for cover, dirty ground and the aftermath of earlier knife violence.
- Action: Barry confronts Dave while Kate's dangerous presence or ice magic shifts the balance.
- Materials/architecture: Barry has ordinary travel gear only; Dave's knives and rough clothes remain central.
- Composition: 16:9 confrontation, Barry visibly scared but determined, Dave threatening, Kate as cold controlled pressure.

Visual direction:
- grounded realistic fantasy adventure illustration
- wide 16:9 landscape, readable on mobile, strong focal point, no UI frame
- cinematic but restrained lighting, coherent scale between characters, detailed materials and environment
- prioritize canonical accuracy and continuity over extra ornament

Avoid:
- No heroic armor for Barry, no later crossbow, no excessive gore.
- no logo, no watermark, no readable text
```
